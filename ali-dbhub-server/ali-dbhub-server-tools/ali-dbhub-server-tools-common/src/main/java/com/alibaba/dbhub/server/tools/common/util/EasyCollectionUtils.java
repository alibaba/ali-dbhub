...
public class EasyCollectionUtils {
    ...
    public static <T> T findFirst(Collection<T> collection) {
        return stream(collection).findFirst().orElse(null);
    }

    /**
     * 将一个集合 转换成一个list
     *
     * <p>会过滤集合中转换前后的空数据，所以会入参出参数量不一致
     *
     * @param collection 集合
    ...
    public static <K, V, T> Map<K, V> toMap(
        ...
        return stream(collection)
                ...
                .collect(
                        Collectors.toMap(
                                keyFunction, valueFunction, (oldValue, newValue) -> newValue));
    }
     ...
     * @return 转成以后的map
     */
    public static <K, T> Map<K, T> toIdentityMap(
            Collection<T> collection, Function<? super T, K> keyFunction) {
    ...
    public static <C> boolean addAll(
            final Collection<C> collection, final Collection<C> collectionAdd) {
     ...
     * @return 去重后的集合
     */
    public static <E, R> List<E> distinctByKey(
            Collection<E> collection, Function<E, R> keyFunction) {
