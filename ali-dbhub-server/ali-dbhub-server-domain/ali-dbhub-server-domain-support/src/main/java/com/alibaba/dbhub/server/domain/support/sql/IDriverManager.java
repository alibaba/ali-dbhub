...
public class IDriverManager {
    ...
    private static final Map<DriverTypeEnum, DriverEntry> DRIVER_ENTRY_MAP =
    ...
    public static Connection getConnection(
            String url, String user, String password, DriverTypeEnum driverTypeEnum)
            throws SQLException {
        Properties info = new Properties();
        if (user != null) {
            info.put("user", user);
    ...
    public static Connection getConnection(
            String url, Properties info, DriverTypeEnum driverTypeEnum) throws SQLException {
        if (url == null) {
        ...
        try {
            ...
            if (con != null) {
                DriverManager.println(
                        "getConnection returning " + driverEntry.getDriver().getClass().getName());
                return con;
            }
        } catch (SQLException var7) {
            Connection con =
                    tryConnectionAgain(driverTypeEnum.getDbTypeEnum(), driverEntry, url, info);
            if (con != null) {
                return con;
            } else {
                throw var7;
    ...
    private static Connection tryConnectionAgain(
            DbTypeEnum dbTypeEnum, DriverEntry driverEntry, String url, Properties info)
            throws SQLException {
    ...
    private static DriverEntry getJDBCDriver(DriverTypeEnum driverTypeEnum) throws SQLException {
        synchronized (driverTypeEnum) {
            try {
                ...
                Driver driver =
                        (Driver) cl.loadClass(driverTypeEnum.getDriverClass()).newInstance();
                DriverEntry driverEntry =
                        DriverEntry.builder().driverTypeEnum(driverTypeEnum).driver(driver).build();
    ...
    public static ClassLoader getClassLoader(DriverTypeEnum driverTypeEnum)
    ...
    // private static List<Class> loadClass(String jarPath, ClassLoader classLoader) throws
    // IOException {
    ...
    //            String className = jarEntry.getName().substring(0, jarEntry.getName().length() -
    // 6).replaceAll("/",
    //                ".");
    //            try {
