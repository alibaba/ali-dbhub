...
public class PostgresqlMetaSchemaSupport extends BaseMetaSchema implements MetaSchema {
    ...
    private String functionSQL =
            " CREATE OR REPLACE FUNCTION showcreatetable(namespace character varying, tablename"
                + " character varying)\n"
                ...
                + "        begin\n"
                + "        -- columns\n"
                + "        tableScript:=tableScript || ' CREATE TABLE '|| tablename|| ' ( '||"
                + " chr(13)||chr(10) || array_to_string(\n"
                + "        array(\n"
                + "        select ' ' || concat_ws(' ',fieldName, fieldType, fieldLen, indexType,"
                + " isNullStr, fieldComment ) as column_line\n"
                + "        from (\n"
                + "        select a.attname as fieldName,format_type(a.atttypid,a.atttypmod) as"
                + " fieldType,(case when atttypmod-4>0 then\n"
                + "        atttypmod-4 else 0 end) as fieldLen,\n"
                + "        (case when (select count(*) from pg_constraint where conrelid ="
                + " a.attrelid and conkey[1]=attnum and\n"
                + "        contype='p')>0 then 'PRI'\n"
                + "        when (select count(*) from pg_constraint where conrelid = a.attrelid and"
                + " conkey[1]=attnum and contype='u')>0\n"
                + "        then 'UNI'\n"
                + "        when (select count(*) from pg_constraint where conrelid = a.attrelid and"
                + " conkey[1]=attnum and contype='f')>0\n"
                + "        then 'FRI'\n"
                + "        else '' end) as indexType,\n"
                + "        (case when a.attnotnull=true then 'not null' else 'null' end) as"
                + " isNullStr,\n"
                + "        ' comment ' || col_description(a.attrelid,a.attnum) as fieldComment\n"
                + "        from pg_attribute a where attstattarget=-1 and attrelid = (select c.oid"
                + " from pg_class c,pg_namespace n where\n"
                ...
                + "        select conname,\n"
                + "        case when contype='c' then ' CHECK('|| ( select"
                + " findattname(namespace,tablename,'c') ) ||')' end as c ,\n"
                + "        case when contype='u' then ' UNIQUE('|| ( select"
                + " findattname(namespace,tablename,'u') ) ||')' end as u ,\n"
                + "        case when contype='p' then ' PRIMARY KEY ('|| ( select"
                + " findattname(namespace,tablename,'p') ) ||')' end as p ,\n"
                + "        case when contype='f' then ' FOREIGN KEY('|| ( select"
                + " findattname(namespace,tablename,'u') ) ||') REFERENCES '||\n"
                + "        (select p.relname from pg_class p where p.oid=c.confrelid ) || '('|| ("
                + " select\n"
                ...
                + "\n"
                + "        -- indexs\n"
                + "        -- CREATE UNIQUE INDEX pg_language_oid_index ON pg_language USING btree"
                + " (oid); -- table pg_language\n"
                + "\n"
                + "\n"
                + "        --\n"
                + "        /** **/\n"
                ...
                + "        -- CREATE UNIQUE INDEX pg_language_oid_index ON pg_language USING btree"
                + " (oid); -- table pg_language\n"
                + "        tableScript:= tableScript || chr(13)||chr(10) || chr(13)||chr(10) ||"
                + " array_to_string(\n"
                + "        array(\n"
                + "        select 'CREATE INDEX ' || indexrelname || ' ON ' || tablename || ' USING"
                + " btree '|| '(' || attname || ');' from (\n"
                + "        SELECT\n"
                ...
                + "        array(\n"
                + "        select a.attname from pg_attribute a where attrelid=c.oid and a.attnum"
                + " in ( select unnest(x.indkey) )\n"
                + "\n"
                + "        )\n"
                + "        ,',' ) )as attname\n"
                + "\n"
                ...
                + " array_to_string(\n"
                + "        array(\n"
                + "        SELECT 'COMMENT ON COLUMN' || tablename || '.' || a.attname ||' IS '||"
                + " ''''|| d.description ||''''\n"
                + "        FROM pg_class c\n"
                ...
                + " tablename character varying, ctype character\n"
                + "        varying)\n"
    ...
    public String tableDDL(String databaseName, String schemaName, String tableName) {
        SQLExecutor.getInstance()
                .executeSql(functionSQL.replaceFirst("tableSchema", schemaName), resultSet -> null);
