...
public class IDataSource extends DruidDataSource {
    @Serial private static final long serialVersionUID = -232274227856574115L;
    ...
    public IDataSource(
            ConnectInfo connectInfo, DriverTypeEnum driverTypeEnum, ClassLoader classLoader) {
    ...
    private void connectSession() {
        ...
        if (ssh != null && ssh.isUse()) {
            ...
            url =
                    url.replace(connectInfo.getHost(), "127.0.0.1")
                            .replace(connectInfo.getPort() + "", ssh.getLocalPort());
            setUrl(url);
        }
    }
}
