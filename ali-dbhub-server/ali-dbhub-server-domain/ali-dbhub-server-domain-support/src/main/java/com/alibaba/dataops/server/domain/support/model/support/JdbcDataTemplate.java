...
import com.alibaba.dataops.server.domain.support.enums.CellTypeEnum;
import com.alibaba.dataops.server.domain.support.model.Cell;
            ...
            if (query) {
                ...
                try {
                    ...
                    while (rs.next()) {
                        ...
                        dataList.add(row);
                        for (int i = 1; i <= col; i++) {
                            row.add(com.alibaba.dataops.server.domain.support.util.JdbcUtils.getResultSetValue(rs, i));
                        }
                        rsSize++;
                        // 到达下一页了
                        if (rsSize >= pageSize) {
