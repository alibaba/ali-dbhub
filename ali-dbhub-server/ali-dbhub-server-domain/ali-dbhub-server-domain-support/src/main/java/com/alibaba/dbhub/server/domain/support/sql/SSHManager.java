...
public class SSHManager {
    ...
    private static final ConcurrentHashMap<SSHInfo, Session> SSH_SESSION_MAP =
    ...
    private static Session createSession(SSHInfo ssh) {
        synchronized (ssh) {
            ...
            if (StringUtils.isNotBlank(ssh.getLocalPort())
                    && StringUtils.isNotBlank(ssh.getRHost())
                    && StringUtils.isNotBlank(ssh.getRPort())) {
                try {
                    int port1 =
                            session.setPortForwardingL(
                                    Integer.parseInt(ssh.getLocalPort()),
                                    ssh.getRHost(),
                    ...
                    throw new RuntimeException(
                            ssh.getLocalPort() + " port is used，please change to another port ", e);
                }
            }
            return session;
        }
