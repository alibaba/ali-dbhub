...
public class SqlUtilsTest {
    ...
    public void test() {
        List<SQLStatement> sqlStatements =
                SQLUtils.parseStatements("select 1 from test;", DbType.mysql);
        log.info("解析sql:{}", sqlStatements);
        sqlStatements =
                SQLUtils.parseStatements(
                        "use xxx;select 1 from test;explain select 1 from test", DbType.mysql);
        log.info("解析sql:{}", sqlStatements);
    ...
    public void test55() {
        List<SQLStatement> sqlStatements =
                SQLUtils.parseStatements("create table test(id int) comment 'xx';", DbType.mysql);
        log.info("解析sql:{}", sqlStatements);
    }
    ...
    public void test2() {
        ...
        log.info(
                "分页:{} ----- {}  ---  {}",
                PagerUtils.count(sql, DbType.mysql),
    ...
    public void test56() {
        SQLStatement sqlStatement =
                SQLUtils.parseSingleStatement(
                        "create table test(id int  ,name varchar(32) not null default 'xx' comment"
                                + " 'name',nu int auto_increment,index ds(id) ,primary key (id,nu))"
                                + " comment 'xx';",
                        DbType.mysql);
    ...
    public void testreaname() {
        SQLStatement sqlStatement =
                SQLUtils.parseSingleStatement(
                        "rename table data_ops_table_test_1667268894825 to"
                                + " data_ops_table_test_166726889482511;",
                        DbType.mysql);
        log.info("解析sql:{}", sqlStatement);
    }
    ...
    public void dropindex() {
        SQLStatement sqlStatement =
                SQLUtils.parseSingleStatement(
                        "drop index data_ops_table_test_1667268894825_idx_date on"
                                + " data_ops_table_test_1667268894825;",
                        DbType.mysql);
        log.info("解析sql:{}", sqlStatement);
    }
    ...
    public void coment() {
        try {
            ...
            SQLStatement sqlStatement =
                    SQLUtils.parseSingleStatement(
                            "comment on index DATA_OPS_TEMPLATE_TEST_1672663574919_idx_date is"
                                    + " '日期索引xx';\n",
                            DbType.h2,
    ...
    public void errro() {
        List<SQLStatement> sqlStatementList =
                SQLUtils.parseStatements(
                        "alter table data_ops_table_test_1667268894825  drop column string;comment"
                                + " on index DATA_OPS_TEMPLATE_TEST_1672663574919_idx_date is"
                                + " '日期索引xx';\n",
                        DbType.h2,
                        SQLParserFeature.PrintSQLWhileParsingFailed);
    ...
    public void spilt() {
        List<String> list =
                MysqlUtils.splitAndRemoveComment(
                        "alter table data_ops_table_test_1667268894825  drop column string;comment"
                                + " on index DATA_OPS_TEMPLATE_TEST_1672663574919_idx_date is"
                                + " '日期索引xx;;;';\n",
                        DbType.h2);
        ...
        List<SQLStatement> sqlStatementList =
                SQLUtils.parseStatements(
                        "CREATE TABLE `data_ops_table_test_1673096155228`\n"
                            ...
                            + "\t    index data_ops_table_test_1673096155228_idx_date (date desc)"
                            + " comment '日期索引',\n"
                            + "\t    unique data_ops_table_test_1673096155228_uk_number (number)"
                            + " comment '唯一索引',\n"
                            + "\t    index data_ops_table_test_1673096155228_idx_number_string"
                            + " (number, date) comment '联合索引'\n"
                            + "\t) COMMENT ='测试表';",
                        DbType.mysql);
        log.info("解析sql:{}", sqlStatementList);
    }
