 ...
 */
@Slf4j
public class ManageApplicationListener
    ...
    public void onApplicationEvent(ApplicationEnvironmentPreparedEvent event) {
        ...
        String environment =
                event.getEnvironment().getProperty("spring.profiles.active", String.class);

        // 尝试访问确认应用是否已经启动
        DataResult<String> dataResult;
        try {
            dataResult =
                    Forest.get("http://127.0.0.1:" + serverPort + "/api/system/get-version-a")
        ...
        ProcessHandle.allProcesses()
                .forEach(
                        process -> {
                            ...
                            // 不是java应用
                            boolean isJava =
                                    StringUtils.endsWithIgnoreCase(command, "java")
                                            || StringUtils.endsWithIgnoreCase(command, "java.exe");
                            if (!isJava) {
                                return;
                            }
                                ...
                                if (StringUtils.startsWith(argument, "-Dspring.profiles.active=")) {
                                    environmentArgument =
                                            StringUtils.substringAfter(
                                                    argument, "-Dspring.profiles.active=");
                                }
                            }
                            // 不是dbhub
                            if (!isDbhub) {
                            ...
                            if (StringUtils.equals(
                                            SystemEnvironmentEnum.RELEASE.getCode(), environment)
                                    && StringUtils.equals(
                                            SystemEnvironmentEnum.RELEASE.getCode(),
                                            environmentArgument)) {
                                log.info("正式环境需要关闭进程");
                            ...
                            // 判断是否是测试环境
                            if (StringUtils.equals(
                                            SystemEnvironmentEnum.TEST.getCode(), environment)
                                    && StringUtils.equals(
                                            SystemEnvironmentEnum.TEST.getCode(),
                                            environmentArgument)) {
                                log.info("测试环境需要关闭进程");
                                destroyProcess(process, command, arguments);
                                return;
                            ...
                            boolean devDestroy =
                                    StringUtils.equals(
                                                    SystemEnvironmentEnum.DEV.getCode(),
                                                    environment)
