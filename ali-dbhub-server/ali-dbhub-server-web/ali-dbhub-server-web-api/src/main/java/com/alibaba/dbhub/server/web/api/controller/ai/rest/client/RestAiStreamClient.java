...
public class RestAiStreamClient {
    /** rest api url */
    @Getter private String apiUrl;

    /** 是否流式接口 */
    @Getter private Boolean stream;
    /** okHttpClient */
     ...
     */
    public void restCompletions(String prompt, EventSourceListener eventSourceListener) {
    ...
    public void streamCompletions(
            RestAiCompletion completion, EventSourceListener eventSourceListener) {
     ...
     * @param eventSourceListener
     */
    public void nonStreamCompletions(
            RestAiCompletion completion, EventSourceListener eventSourceListener) {
        if (StringUtils.isBlank(completion.getPrompt())) {
            log.error("参数异常：Prompt不能为空");
            throw new BusinessException(CommonErrorEnum.PARAM_ERROR);
